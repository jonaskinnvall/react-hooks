...
- Run side effects on re-renders using the React.useEffect hook, such as saving
  state to local storage
- Lazy-initialize state by passing a function to useState on initial render,
  which can be useful in case of a computationally heavy operation has to be
  done
- Use the dependency array to optimize re-renders but also what can go wrong if
  warnings are ignored and it is not handled correctly
- Create custom hooks, which are functions that use React hooks, to break out
  commonly used hooks logic into one place to be used across the application
- Make a custom hook more flexible to account for different types of input from
  components using it
