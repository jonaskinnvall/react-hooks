...
- Utilize what we've learned up until now (useState, useEffect, lift state, to
  use local storage) to implement a small application
- Use our 'managed state' (useState) to get 'derived states' by using the
  'managed state' as a basis to calculate what else we need
- How lifting state can help us implement a new feature to an application
