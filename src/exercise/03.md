...
- The concept of 'Lifting state' which means to lift the state of a component up
  to the closest parent when that state needs to be accessed in a sibling
  component
- The opposite concept of 'colocating state' which is done by moving the state
  down into a child component if that state is not needed anywhere else (e.g. a
  sibling component don't need that state anymore)
